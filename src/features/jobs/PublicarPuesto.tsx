...
export const PublicarPuesto: React.FC = () => {
  ...
  function validate(): boolean {
    ...
    if (!form.titulo.trim()) errs.titulo = "Ingresá un título";
    if (!form.empresa.trim()) errs.empresa = "Ingresá el nombre de la empresa";
    if (!form.contactoEmail.trim() && !form.contactoTelefono.trim())
      errs.contacto = "Proporcioná email o teléfono de contacto";
    ...
    <main className="max-w-3xl mx-auto p-6 bg-white rounded-2xl shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <textarea name="descripcion" value={form.descripcion} onChange={handleChange} rows={6} className="w-full rounded-lg border px-3 py-2 text-sm border-slate-200" placeholder="Describí el puesto, tareas y stack técnico..." />
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2 mb-2">
            <input ref={skillInputRef} onKeyDown={handleKeyDownSkill} className="flex-1 rounded-lg border px-3 py-2 text-sm border-slate-200" placeholder="Escribí y presioná Enter o , para agregar" />
